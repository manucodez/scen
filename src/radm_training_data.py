...
weather_file_path = os.environ.get(
    "WEATHER_FILE",
    os.path.join(data_dir, "weather_batch.yaml")
)

feedback_file_path = os.environ.get(
    "FEEDBACK_FILE",
    os.path.join(data_dir, "feedback_rounds", "weather_feedback_summary_radm_round1.csv")
)

# === 1. Load weather parameters ===
with open(weather_file_path, "r", encoding="utf-8") as f:
    weather_yaml = yaml.safe_load(f)
...
# === 4. Train risk predictor using weather ONLY ===
weather_cols = [c for c in weather_df.columns if c != "weather_id"]

X = merged_df[weather_cols].to_numpy(dtype=np.float32)
